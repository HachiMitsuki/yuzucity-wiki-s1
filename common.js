  ...
  if (hasGsap && slug === 'index'){
    ...
    document.querySelectorAll(`[data-reveal], ${GENERIC_SELECTOR}`).forEach(el => {
      ...
      gsap.timeline({
        ...
        .fromTo(el, from, { ...to, ease: 'none', duration: 0.12 })
        .to(el, { duration: 0.76 }) // hold — no-op, keeps the "to" state while in view
        .to(el, { ...from, ease: 'none', duration: 0.12 }); // fade back out on the way past
    });
    ...
    document.querySelectorAll(HEADING_SELECTOR).forEach(heading => {
      ...
      gsap.timeline({
        ...
        .fromTo(charEls,
          ...
          { opacity: 1, x: 0, y: 0, ease: 'none', stagger: 0.025, duration: 0.12 })
        .to(charEls, { duration: 0.76 })
        .to(charEls, { opacity: 0, x: (i) => charFrom(i).x, y: (i) => charFrom(i).y, ease: 'none', stagger: 0.025, duration: 0.12 });
    });
